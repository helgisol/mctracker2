createSeeds <- function(tconf, tstate, updClusters)
{
  ...
  histObj <- calcSeedHistObj(tconf, tstate, incompleteClusterObjs, nonconsistentCmpObs, newObs)
  seeds$dw <- calcDistWeightMap(tconf, seeds, histObj) # Calculate distance weight map.
